...
func (this *FileService) GetFileBase64(userId, fileId string) (str string, mine string) {
	...
	e64 := base64.StdEncoding
	maxEncLen := e64.EncodedLen(len(bts))
	encBuf := make([]byte, maxEncLen)
	e64.Encode(encBuf, bts)
...
func (this *FileService) CopyImage(userId, fileId, toUserId string) (bool, string) {
	...
	_, ext := lea.SplitFilename(file.Name)
	guid := lea.NewGuid()
	newFilename := guid + ext

	// TODO 统一目录格式
	// dir := "files/" + toUserId + "/images"
	dir := fmt.Sprint("oss/files/", toUserId, "/images/", time.Now().Year(), "/", time.Now().Month(), "/", time.Now().Day() )
	dstFilePath := dir + "/" + newFilename

	mc, _ := GetMinioClient()
	b , _ := mc.CopyObject(dstFilePath, strings.TrimLeft(file.Path, "/"))
	if !b {
		return false, ""
	}
