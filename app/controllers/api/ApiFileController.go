...
func (c ApiFile) GetAttach(fileId string) revel.Result {
	...
	bts, _, b := mc.GetObject(strings.TrimLeft(path, "/"))
	if b {
		return c.RenderBinary(bytes.NewReader(bts), attach.Title, revel.Attachment, time.Now()) // revel.Attachment
	}
	return c.RenderText("Get File Error !")
}
...
func (c ApiFile) GetAllAttachs(noteId string) revel.Result {
	...
	// 遍历文件列表
	for _, attach := range attachs {
		objInfo := mc.StatObject(strings.TrimLeft(attach.Path, "/"))
		bts, _ , b := mc.GetObject(strings.TrimLeft(attach.Path, "/"))
		if !b {
			return c.RenderText("")
		}
		// 信息头
		...
		h.Size = objInfo.Size
		h.Mode = int64(0755)
		h.ModTime = objInfo.LastModified
		...
		// 写文件
		_, err = io.Copy(tw, bytes.NewReader(bts))
		if err != nil {
			panic(err)
		}
	} // for
