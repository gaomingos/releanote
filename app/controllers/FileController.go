...
func (c File) UploadAvatar() revel.Result {
	...
	c.ViewArgs["fileUrlPath"] = re.Id //service.PresignedGebject(re.Id, 360)
	c.ViewArgs["resultCode"] = re.Code
	c.ViewArgs["resultMsg"] = re.Msg
...
func (c File) uploadImage(from, albumId string) (re info.Re) {
	...
	if from == "logo" || from == "blogLogo" {
		fileUrlPath = fmt.Sprint("oss/upload/", userId, "/", time.Now().Year(), "/", time.Now().Month(), "/", time.Now().Day(), "/logo" )
	} else {
		fileUrlPath = fmt.Sprint("oss/files/", userId, "/", time.Now().Year(), "/", time.Now().Month(), "/", time.Now().Day() )
	...
	objectName := fmt.Sprint(fileUrlPath, "/" ,filename)
	mc, _ := service.GetMinioClient()
	_, b := mc.PutObject(data, objectName, contentType)
	if !b {
		return re
	}
	resultCode = 1
